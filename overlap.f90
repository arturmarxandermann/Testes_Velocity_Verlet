...
!subrotina que define todos os sitios da celula OPV
    ...
    site%radius  = InitSitesRadius
    site%vel     = InitSitesVel 
    ...
    do j = 1 , ns_el
      do i = 1 , ns_el
        ...
        select case (Q_numbers)
          ...
          case(0010)
          SMtx(i, j) = -2.d0*SQRT2*costh*d*EXPd2*sqmhbar*w1_15*w2*wden4
          ...
          case(1000)
          SMtx(i, j) = 2.d0*SQRT2*costh*d*EXPd2*sqmhbar*w1*w2_15*wden4
...
subroutine DerivativeOverlap(s1, s2, DerOvlp)        
    ...
    real*8    :: wSquareDiff, twow1Minusw2, threew1Minusw2
    ...
    wSquareDiff = w2_2 - w1_2 
    twow1Minusw2 = 2.d0 * w1 - w2
    threew1Minusw2 = 3.d0 * w1 - w2 
    ...
    do j = 1 , ns_el
      do i = 1 , ns_el
        ...
        case(0000)
          DerOvlp(i, j) = -((EXPd2*w1Sw2half*(d2*me*w1_2*w2 + hbar*(wSquareDiff))*wden6)/hbar)
        ...
        case(0010)
          DerOvlp(i, j) =  SQRT2*costh*d*EXPd2*Sqrt(me)*(w1/hbar)**1.5d0*((d2*me*w1_2*w2) - &
                            2.d0*hbar*(wSquareDiff))*wden8 
          
        case(0100) 
          DerOvlp(i, j) = -((SQRT2*d*EXPd2*me*sinth*w1*Sqrt(w2)*(-(hbar*omega1PlusOmega2*sqhbarm*(threew1Minusw2)) + & 
                             d2*Sqrt(hbar*me)*w1_2*w2)*wden8)/hbar2 )        
          
        case(0101)
          DerOvlp(i, j) = (2.d0*EXPd2*w1*wden10*(2.d0*hbar2*omegaMinusPlus2 + d4*me2*sinth2*w1_3*w2_2 + &
                            d2*hbar*me*omega1PlusOmega2*(-3.d0*w1 + cos2th*(twow1Minusw2) + w2)*wproduct2))/hbar2
          
          
        case(0110)
          DerOvlp(i, j) = (d2*EXPd2*me*sin2th*w1_2*w2*(-2.d0*hbar*omega1PlusOmega2*(twow1Minusw2) + &
                            d2*me*w1_2*w2)*wden10)/hbar2
          
          
        case(1000)
          DerOvlp(i, j) = -((SQRT2*EXPd2*costh*d*w1*Sqrt(me*w2)*(-(hbar*omega1PlusOmega2*(threew1Minusw2)) + &
        ...
        case(1010)
          DerOvlp(i, j) = (2.d0*w1*EXPd2*wden10*(costh2*d4*me2*w1_3*w2_2 + hbar*omega1PlusOmega2*(-2.d0*hbar*(wSquareDiff) - & 
                            d2*me*(threew1Minusw2)*wproduct2 - cos2th*d2*me*(twow1Minusw2)*wproduct2)))/hbar2 
