...
!com rhoreal_aa
!seu tamanho é d_el. Os outros blocos tem tamanho non_diag pois relacionam os
!elementos
!de antes com rhoreal_ab, rhoimag_ab

    ! args
    REAL*8,              INTENT(IN)    :: omega_matrix(d_el, d_el) 
    ...
    non_diag   = (d_el * ( d_el -1 ) ) / 2
    rhosquared = d_el * d_el
    aux1       = d_el + non_diag
    
    
    ALLOCATE( Rw_matrix(rhosquared, rhosquared)        , source = 0.d0 )
    ALLOCATE( Rw_blocks(1)%elements(d_el, d_el)    , source = 0.d0 )
    ALLOCATE( Rw_blocks(2)%elements(d_el, non_diag)  , source = 0.d0 )
    ALLOCATE( Rw_blocks(3)%elements(d_el, non_diag)  , source = 0.d0 )
    ALLOCATE( Rw_blocks(4)%elements(non_diag, d_el)  , source = 0.d0 )
    ALLOCATE( Rw_blocks(5)%elements(non_diag, non_diag), source = 0.d0 )
    ALLOCATE( Rw_blocks(6)%elements(non_diag, non_diag), source = 0.d0 )
    ALLOCATE( Rw_blocks(7)%elements(non_diag, d_el)  , source = 0.d0 )
    ...
    Rw_matrix(1:d_el, 1:d_el)                   = Rw_blocks(1)%elements
    Rw_matrix(1:d_el, d_el+1:aux1)              = Rw_blocks(2)%elements
    Rw_matrix(1:d_el, aux1+1:rhosquared)          = Rw_blocks(3)%elements
    
    Rw_matrix(d_el+1:aux1, 1:d_el)              = Rw_blocks(4)%elements
    Rw_matrix(d_el+1:aux1, d_el+1:aux1)         = Rw_blocks(5)%elements
    Rw_matrix(d_el+1:aux1, aux1+1:rhosquared)     = Rw_blocks(6)%elements
    
    Rw_matrix(aux1+1:rhosquared, 1:d_el)          = Rw_blocks(7)%elements
    Rw_matrix(aux1+1:rhosquared, d_el+1:aux1)     = Rw_blocks(8)%elements
