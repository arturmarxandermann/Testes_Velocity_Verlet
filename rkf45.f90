...
!
 ...
 call f ( t + ch, f5, f1 )
  ...
  f5(i) = y(i) + ch * &
  ( 1932.0D+00 * yp(i) &
  + ( 7296.0D+00 * f2(i) - 7200.0D+00 * f1(i) ) &
  )
  enddo
  ...
  f5(i) = y(i) + ch * &
  ( &
    ( 8341.0D+00 * yp(i) - 845.0D+00 * f3(i) ) &
  + ( 29440.0D+00 * f2(i) - 32832.0D+00 * f1(i) ) &
  )
  enddo
 ...
 do i = 1, neqn
  ...
  ( &
    ( -6080.0D+00 * yp(i) &
    + ( 9295.0D+00 * f3(i) - 5643.0D+00 * f4(i) ) &
    ) &
  + ( 41040.0D+00 * f1(i) - 28352.0D+00 * f2(i) ) &
  )
 enddo
...
!
 ...
 !DIR$ PARALLEL
  ...
  ( &
    ( 902880.0D+00 * yp(i) &
    + ( 3855735.0D+00 * f3(i) - 1371249.0D+00 * f4(i) ) ) &
  + ( 3953664.0D+00 * f2(i) + 277020.0D+00 * f5(i) ) &
  )
enddo
